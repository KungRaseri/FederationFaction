...
function foundFactory(goodName, productionIndex)
    ...
    if asteroid.factionIndex ~= callingPlayer then return end
    ...
    if production == nil then
        player:sendChatMessage("Station Founder"%_t, 1, "The production line you chose doesn't exist."%_t)
        return
    end
    ...
    local station = transformToStation(asteroid, "${good} Mine"%_t % {good = goodName})
...
function transformToStation(asteroid, name)
    ...
    local player = Player(asteroid.factionIndex)
    ...
    desc.factionIndex = player.index
    desc:setPlan(plan)
