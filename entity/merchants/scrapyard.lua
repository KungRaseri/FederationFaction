...
function initUI()
    ...
    priceLabel1 = salvageTab:createLabel(vec2(200, 85),  "$ " .. getLicensePrice(Player(), 5),  fontSize)
    priceLabel2 = salvageTab:createLabel(vec2(200, 135), "$ " .. getLicensePrice(Player(), 15), fontSize)
    priceLabel3 = salvageTab:createLabel(vec2(200, 185), "$ " .. getLicensePrice(Player(), 30), fontSize)
    priceLabel4 = salvageTab:createLabel(vec2(200, 235), "$ " .. getLicensePrice(Player(), 60), fontSize)
...
function onShowWindow()
    ...
    priceLabel1.caption = "$${money}"%_t % {money = getLicensePrice(Player(), 5)}
    priceLabel2.caption = "$${money}"%_t % {money = getLicensePrice(Player(), 15)}
    priceLabel3.caption = "$${money}"%_t % {money = getLicensePrice(Player(), 30)}
    priceLabel4.caption = "$${money}"%_t % {money = getLicensePrice(Player(), 60)}

    uiMoneyValue = getShipValue(plan)
...
function onSellButtonPressed()
    invokeServerFunction("sellCraft", Player().craftIndex)
end

function onBuyLicenseButton1Pressed()
    invokeServerFunction("buyLicense", 60 * 5)
end

function onBuyLicenseButton2Pressed()
    invokeServerFunction("buyLicense", 60 * 15)
end

function onBuyLicenseButton3Pressed()
    invokeServerFunction("buyLicense", 60 * 30)
end

function onBuyLicenseButton4Pressed()
...
function getLicenseDuration()
    invokeServerFunction("sendLicenseDuration")
end

function setLicenseDuration(duration)
...
function getLicensePrice(orderingFaction, minutes)
...
function sellCraft(craftIndex)
    ...
    local moneyValue = getShipValue(plan)
    player:receive(moneyValue)

    invokeClientFunction(player, "transactionComplete")
end

function getShipValue(plan)
...
function buyLicense(duration)

    local player = Player(callingPlayer)
    local price = getLicensePrice(player, duration / 60) -- minutes!
    ...
    licenses[callingPlayer] = duration

    -- send a message as response
    local minutes = licenses[callingPlayer] / 60
    ...
    sendLicenseDuration(callingPlayer)
end

function sendLicenseDuration()
...
function onHullHit(objectIndex, block, shootingCraftIndex, damage, position)
    ...
    if object.isWreckage then
        ...
        if shooter then
            ...
            if faction.isPlayer and licenses[faction.index] == nil then
                unallowedDamaging(Player(faction.index), damage)
            end
        end
    end
end

function unallowedDamaging(player, damage)

    local station = Entity()

    local actions = illegalActions[player.index]
    if actions == nil then
        actions = 0
    end
    ...
    if actions < 10 and newActions >= 10 then
        player:sendChatMessage(station.title, 0, "Salvaging or damaging wreckages in this sector is illegal. Please buy a savaging license."%_t);
    ...
    if actions < 500 and newActions >= 500 then
        player:sendChatMessage(station.title, 0, "Wreckages in this sector are the property of %s. Please buy a savaging license."%_t, Faction().name);
        player:sendChatMessage(station.title, 2, "You need a salvaging license for this sector."%_t);
    end
    ...
    if newActions > 5 then
        Galaxy():changeFactionRelations(Faction(), player, -newActions / 100)
    end

    illegalActions[player.index] = newActions
